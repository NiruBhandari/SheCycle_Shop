...
function sendMail($email, $v_code)
{
    ...
    try {
        ...
        $mail->Body = "Thanks for registering!<br><br>Click the link below to verify your email address:<br><a href='http://localhost/Shecycle/Buyer/verifyb.php?email=$email&v_code=$v_code'>Verify</a>";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Prepare and bind
    $stmt = $conn->prepare("INSERT INTO buyers (name, password, email, gender, location, province, city, district, verification_code, is_verified) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    if ($stmt->execute()) {
        ...
        header("Location: BuyerLogin.php?message=" . urlencode('Registration successful! A verification email has been sent to email!'));
        exit();
